...
// requires: sin ninguno de esos módulos disponibles, el cargo no tiene sentido en esta empresa y no se
// ofrece (por ejemplo, Repartidor en un plan sin envíos a domicilio).
export const ROLE_OPTIONS = [
  { value: 'VENDEDOR', label: 'Vendedor', hint: 'Mostrador / ventas', badge: 'bg-orange-100 text-orange-700 border-orange-200', requires: ['pos'] },
  { value: 'CAJERO', label: 'Cajero', hint: 'Cobro y arqueos', badge: 'bg-emerald-100 text-emerald-700 border-emerald-200', requires: ['caja'] },
  { value: 'ALMACEN', label: 'Almacén', hint: 'Productos, compras, kardex y despacho', badge: 'bg-amber-100 text-amber-800 border-amber-200', requires: ['inventory', 'kardex', 'compras', 'despacho'] },
  { value: 'REPARTIDOR', label: 'Repartidor', hint: 'Envíos a domicilio', badge: 'bg-blue-100 text-blue-700 border-blue-200', requires: ['deliveries'] },
  { value: 'ADMINISTRADOR', label: 'Administrador', hint: 'Control total', badge: 'bg-purple-100 text-purple-700 border-purple-200', requires: [] },
];

// Cargos que tienen sentido con los módulos que la empresa usa (contratados y activos).
export const rolesForModules = (availableModules) =>
  ROLE_OPTIONS.filter(role => role.requires.length === 0 || role.requires.some(m => availableModules.includes(m)));
...
// Solo se sugiere lo que la empresa tiene disponible: lo que no está en su plan no se marca.
export function presetModules(role, branch, availableModules) {
  ...
  const sugeridos = (() => {
    switch (role) {
      ...
      case 'ADMINISTRADOR': return [...availableModules];
      default: return ['pos'];
    }
  })();
  return sugeridos.filter(m => availableModules.includes(m));
}
